...
full_data <- add_chords_positions(tracks_and_lyrics)
```
...
opisowe <- full_data %>% summarise(srednia= mean(instrumentalness), mediana = median(instrumentalness),
                                   var = var(instrumentalness), odchylenie_st = sd(instrumentalness),
                                   min = min(instrumentalness), max = max(instrumentalness),
                                   Q1 = quantile(instrumentalness, 0.25), Q3 = quantile(instrumentalness, 0.75))

opisowev2 <- full_data %>% summarise(skosnosc = skewness(instrumentalness), kurtuoza = kurtosis(instrumentalness))
...
dists_chord <- get_dists_chords(full_chords)
...
song_dist <- get_dists(words_1$lemma)
...
test <- data.frame(a1= c(rnorm(20, 12, 3)), a2 =c(rep(2, 5), rep(3,5), rep(5, 5), rep(7, 5)))
test %>% count(a2, a1)
test_v <- vector(mode = "character")

test_v <- append(test_v, c("a", "b", "f"))
