...
tracks_lyrics <- add_lyrics(tracks_final)
```
...
get_alb_dt_imp <- function(artist, album) {
  tryCatch(get_album_data(artist, album), error = function(e) {return(NA)}) 
}
add_lyrics <- function(x) {
  artist_album <- x %>% distinct(artist_name, album_name, album_id)
  every_lyrics <- data.frame(lyrics = NULL)
  for(i in 1:nrow(artist_album)) {
    data_lyrics <- get_album_data(artist_album$artist_name[i], artist_album$album_name[i])
    album_lyrics <- data_lyrics %>% filter(album_id == artist_album$album_id[i]) %>% select(lyrics)
    every_lyrics <- rbind(every_lyrics, album_lyrics)
  }
  x <- cbind(x, every_lyrics)
  return(x)
}
tracks_final %>% distinct(artist_name, album_name, album_id)
get_album_data("Sia", "The Girl You Lost", )
```
